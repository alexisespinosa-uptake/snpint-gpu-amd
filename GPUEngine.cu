...
static hipDeviceProp_t gpuProps;
...
void GPUEngine::initialize() {
    ...
    if(gpuProps.multiProcessorCount == 0) {
        checkCUDAError(hipGetDeviceProperties(&gpuProps, 0))
    }
    ...
    checkCUDAError(hipMalloc(&devTables, allocsize))
    checkCUDAError(hipMalloc(&devResults, resultBufferSize))
    // we do not need more memory, so reserve half of the total mem as heap
    checkCUDAError(hipDeviceSetLimit(hipLimitMallocHeapSize, gpuProps.totalGlobalMem/2))
...
void GPUEngine::uploadSNPData() {
    ...
    checkCUDAError(hipMemcpy(devDB, snpdb.data(), snpdb.getBufferSize(), hipMemcpyHostToDevice))
...
//    std::cerr << "Heap size: " << heapsize << "\nbs*nb*2*snpsize: " << (blockSize * numBlocks * 2 * snpdb.getSNPSize()) << "\nte*2*snpsize: " << (idsExpected * 2 * snpdb.getSNPSize()) << std::endl;
    ...
    checkCUDAError(hipMemcpy(devTables, source, idsExpected * idSize, hipMemcpyHostToDevice))
    ...
    checkCUDAError(hipMemcpy(results, devResults, resultView.getResultCount() * resultView.getResultSize(), hipMemcpyDeviceToHost))
    hipDeviceSynchronize();
}
